...
export class QueueService {
  ...
  constructor(private configService: ConfigService) {
    ...
    const workersCount = parseInt(process.env.COUNT_OF_WORKERS_THREE) || 1;
    ...
    for (let i = 0; i < workersCount; i++) {
      ...
      worker.on('completed', (job) => {
        this.logger.log(
          `Worker ${i}, Microservice 3: Job ${job.id} has completed!`,
        );
      });

      worker.on('failed', (job, err) => {
        this.logger.log(
          `Worker ${i}, Microservice 3: Job ${job.id} has failed with ${err.message}`,
        );
      });
